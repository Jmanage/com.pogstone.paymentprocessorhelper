...
class CRM_Paymentprocessorhelper_Form_Search_PaymentProcessorMessages extends CRM_Contact_Form_Search_Custom_Base implements CRM_Contact_Form_Search_Interface {
  ...
  function buildForm(&$form) {
    ...
    $form->add('select', 'status_choices', ts('Transaction Status'), $status_choices, FALSE);
    ...
    $form->add('select', 'recur_choice', ts('Only Recurring or Not'), $recur_choice_opts, FALSE);
    ...
    $form->add('select', 'tran_type_choices', ts('Transaction Types(Auth.net only)'), $tran_type_choices, FALSE);
    ...
    $form->add('select', 'payment_processor_type_choices', ts('Payment Processor Type'), $processor_choices, FALSE);
    ...
    /*
      ...
      }elseif($contrib_id_choice == 'missing_contrib_id'){
    ...
    $form->add('select', 'contrib_id_choice', ts('Association with Contribution'), $contrib_id_choice, FALSE);
    ...
    $form->add('select', 'layout_choice', ts('Layout Choice'), $layout_choice, FALSE);

    $form->add('text', 'transaction_id', ts('Transaction ID'), FALSE
    );

    $form->add('text', 'subscription_id', ts('Processor Subscription ID'), FALSE
    );

    $form->add('text', 'amount', ts('Amount'), FALSE
    );

    $form->add('text', 'first_name', ts('Contact First Name'), FALSE
    );

    $form->add('text', 'last_name', ts('Contact Last Name'), FALSE
    );
  ...
  function setColumns() {
    ...
    elseif ($processor_type == "authorize.net") {
  ...
  function where($includeContactIDs = FALSE) {
    ...
    if ($processor_type == 'eway') {
      ...
      elseif ($tran_type_choices == 'auth_capture') {
    ...
    if ($processor_type == 'eway') {
      ...
      elseif ($recur_choice == 'nonrecur_only') {
